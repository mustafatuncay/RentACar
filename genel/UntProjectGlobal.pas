...
uses
  ...
  Mask, Db, DBClient, ADODB,shellapi,
...
Function BuyukHarf(key1:char;TmpEdit:TEdit):char;
Function BuyukHarfMemo(key1:char;TmpEdit:TMemo):Char;
Function BuyukHarfCombo(key1:char;TmpEdit:TComboBox):Char;
...
Kullanici:string;
KullaniciAdi:string;
ProgramKullanmaDurumu,ProgramKullanmaDurumuTest:integer;
...
if (TmpEdit.SelStart = 0) or
(TmpEdit.SelTEXT = ' ') then

Case Key1 of
'ý': Key1 := 'I';
'i': Key1:= 'Ý';
else Key1 := AnsiUpperCase(Key1)[1];
end
else
if (strng[Length(TmpEdit.Text)]=' ') or (strng[Length(TmpEdit.Text)]='.') then
Case Key1 of
'ý': Key1 := 'I';
'i': Key1 := 'Ý';
else Key1 := AnsiUpperCase(Key1)[1];
...
if NOT (strng[Length(TmpEdit.Text)]=' ') or NOT (strng[Length(TmpEdit.Text)]='.') then
Case Key1 of
'ý': Key1 := 'I';
'i': Key1 := 'Ý';
else Key1 := AnsiUPPERCase(Key1)[1];
